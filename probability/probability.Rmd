...
Answer: Streak length of 1 : M|H M   ..|1
        Streak length of 0 : M|M     ..|1
...
Answer: Kobe's longest streak of baskets is 4, typical streak length is 0, 1
...
```{r load-ggplot, eval=TRUE}
library('tidyverse')
```


```{r calc-streak-kobe-sim, eval=TRUE}
sim_streak <- calc_streak(sim_basket)
ss <- data.frame(table(sim_streak))
ks <- data.frame(table(kobe_streak))

colnames(ss) <- c("streak", "freq")
colnames(ks) <- c("streak", "freq")
ss <- data.frame(ss, grp = rep('sim', nrow(ss)))
ks <- data.frame(ks, grp = rep('kobe', nrow(ks)))
streaks <- rbind(ss, ks)
ggplot(data=streaks, aes(x=streak, y=freq, fill=grp)) + geom_bar(stat="identity", position=position_dodge())
```
...
Answer: Somewhat similar as the model is unchanged and there is random in the process
...
Answer: Kobe Bryant's distribution of streak lengths is very similar to the 
    distribution of streak lengths for the simulated shooter, therefore we do 
    NOT have evidence that the hot hand model fits Kobe's 
    shooting patterns. Goodness of fit using chi-square could be used to confirm 
    that observation.
    
```{r chi-square-distribution, eval=TRUE}
set.seed(42)
pvalue <- rep(0, 1000)
for(i in 1: 1000){
sim_basket <- sample(outcomes, size = 133, replace = TRUE, prob = c(0.45, 0.55))
sim_streak <- calc_streak(sim_basket)
ss<-data.frame(sim_streak, grp = rep('sim', length(sim_streak)))
ks<-data.frame(kobe_streak, grp = rep('kobe', length(kobe_streak)))
colnames(ss) <- c("streak", "grp")
colnames(ks) <- c("streak", "grp")
streaks <-rbind(ss, ks)
output <- chisq.test(streaks$grp, streaks$streak, simulate.p.value = TRUE)
pvalue[i]<- output$p.value
}
sum(pvalue < 0.05)/1000
```
Based on the simulation the `p-value` is only 4 % of the time is below 0.05. Therefore it is 
very likely that Kobe Bryant's distribution of 
streak lengths is very similar to the distribution of streak lengths for the simulated shooter 
