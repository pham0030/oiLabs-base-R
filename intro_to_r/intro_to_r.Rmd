...
```{r plot-prop-boys-vars, eval=FALSE}
boys_prop<- arbuthnot$boys / (arbuthnot$boys + arbuthnot$girls)
plot(x = arbuthnot$year, y = boys_prop, type = "l")
```
...
```{r summary-present-data, eval=FALSE}
summary(present)
```
-   How do these counts compare to Arbuthnot's? Are they on a similar scale?

Answer: No, much larger than Arbuthnot's
...
```{r plot-prop-boys-vars-us, eval=FALSE}
boys_prop<- present$boys / (present$boys + present$girls)
plot(x = present$year, y = boys_prop, type = "l")
``` 
